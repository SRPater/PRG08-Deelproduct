namespace Core
{
    export class GameObject
    {
        ...
        protected collider: Collision.Collider;
        ...
        constructor(public position:Vector2, public width:number, public height:number, needsInput:Boolean = false, collider:Boolean = false, hasGravity:Boolean = false, canMove:Boolean = false, type:E_COLLIDER_TYPES = E_COLLIDER_TYPES.PROP)
        {
            ...
            if(this.hasCollider)
                this.collider = new Collision.BoxCollider(position, width, height, type);

            if(this.hasGravity)
                this.gravity = true;
        }
        
        public isColliding(r: GameObject) : Collision.ColliderReturnObject
        {
        ...
        public collided(co: Collision.CollidedReturnObject) {}
